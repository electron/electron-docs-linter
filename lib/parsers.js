    ...
    .map((i, el) => {
      ...
      if (ret.type === 'Function') {
        ...
        if (innerParams.length === 1) {
          parameters = generateObjectProps($, innerParams, api)
        } else {
          return api.addError('function parameters missing', parameterPattern, $(el))
        }
      }
    ...
    return mSwath
      ...
      .map((i, el) => {
        ...
        if (arg.type === 'Function') {
          arg.parameters = generateObjectProps($, $(el).find('> ul'), api, true)
        }
