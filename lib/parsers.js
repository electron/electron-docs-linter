...
module.exports = {
  ...
  description: ($, openingHeadingElement) => {
    return textify($, swath($, openingHeadingElement)
  ...
  returns: ($, openingHeadingElement, api) => {
    const returnTypePattern = /^Returns <code>([A-Za-z]+)<\/code>/g
    const returnPara = swath($, openingHeadingElement)
      .filter((i, el) => $(el).get(0).tagName === 'p')
      .filter((i, el) => $(el).html().match(/^Returns <code>[A-Za-z]+<\/code>/))
    ...
    if (returnText) {
      type = returnTypePattern.exec(returnHTML)[1]
      description = returnText.split(`Returns ${type}`)[1].replace(/^ - /g, '').trim()
    ...
    if (type === 'Object') {
      ...
      if (props) {
        console.log($(openingHeadingElement).text())
  ...
  parameters: ($, openingHeadingElement, api) => {
    ...
    return mSwath
      .filter((i, el) => {
        ...
        if ($($(mSwath).get(i - 1)).html().match(/^Returns <code>[A-Za-z]+<\/code>/)) return false
        return true
      })
      .first()
      .find('> li')
      .map((i, el) => {
        ...
        if (arg.type === 'Object') {
          ...
          // if (debug) console.log(arg.properties)
        }
