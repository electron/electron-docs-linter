...
const getInnerType = (type) => {
  if (typeof type === 'string') {
    ...
    if (inner) {
      // Handle multi-inners
      inner = inner.split(', ').map(t => {
        const multiType = multitypify(t)
        const isArray = typeof multiType === 'string' && multiType.endsWith('[]')
        if (isArray) {
          return {
            type: stripArrTags(multiType),
            collection: typeof multiType === 'string' && multiType.endsWith('[]')
          }
        }
        return stripArrTags(multiType)
      })
    }
    return { outer, inner }
  }
...
module.exports = {
  ...
  returns: ($, openingHeadingElement, api) => {
    ...
    const innerUsesObject = typeInfo.inner && typeInfo.inner.filter(inner => typeof inner === 'string' && inner.toLowerCase() === 'object').length === 1
    if (type === 'Object' || innerUsesObject) {
      properties = []
      const props = swath($, openingHeadingElement)
        .filter((i, el) => $(el).get(0).tagName === 'ul')
      if (props) {
        properties = generateObjectProps($, props.slice(innerUsesObject ? 1 : 0), api)
      }
    }
