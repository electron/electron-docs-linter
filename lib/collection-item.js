  ...
  getArguments () {
    ...
    return this.$openingHeadingElement
      ...
      .map((i, el) => {
        ...
        // Parsing error
        if (!match) return this.api.addError('arguments', pattern, this.$(el).html())
        ...
        arg.properties = this.$(el)
          .find('li')
          .map((i, el) => {
            var match = this.$(el).html().match(pattern)
            if (!match) return this.api.addError('argument subproperties', pattern, this.$(el).html())
            return {
              name: match[1],
              type: match[2],
              description: match[3]
